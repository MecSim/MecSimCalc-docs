...
Alternatively, the _**developer**_ can choose to use the function `main` in the **Code** stage to return a latex expression as a string (with the the escape character repeated "\\\") or a [raw string](https://www.journaldev.com/23598/python-raw-string) (which precludes the need to repeat the escape character) and then using this output variable in place of "latex expression".
